...
class RiskAgent:
    def __init__(self, *, minimum_score, max_trade_notional,
                 ...
                 paper_bankroll=500.0, max_risk_per_trade_pct=0.01):
    ...
    def assess(self, *, score, positions, daily_pnl, has_open_order=False,
        ...
        if not 0 < self.max_risk_per_trade_pct <= 1:
            return RiskDecision(False, 0.0, 'invalid per-trade risk fraction')
        if stop_loss_pct is not None:
            if (isinstance(stop_loss_pct, bool) or not math.isfinite(float(stop_loss_pct))
                    or not 0 < float(stop_loss_pct) < 1):
                return RiskDecision(False, 0.0, 'invalid stop-loss percentage')
        ...
        risk_cap_notional = float("inf")
        if stop_loss_pct is not None:
            risk_budget = effective_equity * self.max_risk_per_trade_pct
            risk_cap_notional = risk_budget / float(stop_loss_pct)
        notional = round(min(self.max_trade_notional * confidence, available,
                             risk_cap_notional), 2)
        if notional < 1.0:
